...
def diagonal_single_block(N, h, c, phi, J1, J2, I, current_j):
    """
    Creates the diagonal of the Hamiltonian.
    ...
    for i, b in enumerate(basis_set):
        ...
        diag[i] += 0.25 * J1 * (same_pairs - diff_pairs)
        ...
        if not I <= 1:
            ...
            diag[i] += 0.25 * J2 * (same_pairs - diff_pairs)

        # ***Field Terms***
        diag[i] += 0.5 * sum([d[m] for m in b] * field)
    return sp.diags(diag, 0, dtype=complex)


def off_diagonal_single_block(N, J1, J2, I, current_j):
    """
    Creates the off diagonals of the Hamiltonian.
    ...
    blksize = len(basis_set)
    off_diag = sp.lil_matrix((blksize, blksize), dtype=complex)

    # Pairs of elements to inspect. I_pairs = pairs I sites apart
    #  adj_pairs = pairs adjacent to each other
    I_pairs = [(N + i if i < 0 else i, i + I) for i in range(-I, N - I)]
    adj_pairs = [(i, i + 1) for i in range(N - 1) if not (i + 1) % I == 0]
